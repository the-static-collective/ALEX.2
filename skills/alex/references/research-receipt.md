# Research receipt

Use the full receipt for dossiers, audits, PRESSURE runs, reusable research
handoffs, formation-trace runs, or claims whose source chain may later be
challenged. Scale it down for ordinary answers.

## Human-readable template

```markdown
# ALEX research receipt

## Ground
- Question:
- Desired consequence:
- Stop condition:
- Corpus, date, language, and geography:
- Authority and effect boundary:
- Task shape: FIND | READ | COMPARE | TRACE | DOSSIER | AUDIT | PRESSURE
- Formation trace active: yes | no

## World cut
- Included sources and exact heads or versions:
- Deliberately omitted doors:
- Missing or inaccessible sources:
- Sufficiency: sufficient | unresolved

## Acquisitions
| ID | Provider | Item and locus | Method/time | Resolution | Rights/egress |
| --- | --- | --- | --- | --- | --- |

## Witnesses and readings
| ID | Layer | Derived from | Producer/version | Status | Uncertainty |
| --- | --- | --- | --- | --- | --- |

## Claims
| ID | Claim | Class | Supporting evidence path | Counterevidence | Status |
| --- | --- | --- | --- | --- | --- |

## Contradictions and alternatives
- (record entries here)

## Hypothesis lineage — PRESSURE only
- H0 verbatim seed:
- H1 literalized claim:
- H2 corrected survivor:
- H3 cross-domain survivor, if earned:
- What changed at each transition and why:

### Hypothesis-loss receipts
| Transition | What survived | What was removed | Why removed | Recoverable from prior state |
| --- | --- | --- | --- | --- |

## Discovery trace — when formation matters
| ID | From | To | Move | Role | Reason / producer / order |
| --- | --- | --- | --- | --- | --- |

Suggested roles:
- motive
- evidence
- counterevidence
- inference
- analogy
- coincidence
- wordplay
- discriminator

## Toast-ghosts — inactive formation residue
| ID | What was here | Why it left active path | Consequence that remains | Revisitable |
| --- | --- | --- | --- | --- |

## SLEEP suspension — when used
| State | Formulation | Supporting paths | Contradicting paths | Unknowns | Next discriminator | Status |
| --- | --- | --- | --- | --- | --- | --- |

Rules:
- keep 1–3 live states;
- 3 is a cap, not a quota;
- equilibrium does not imply equal confidence;
- killed states leave attributable residue but do not remain live for symmetry.

## .LEEP replay — when used
- Receipt replayed:
- Result: EXACT REPLAY | ATTRIBUTABLE RECONSTITUTION | DIVERGENT REPLAY | INSUFFICIENT RECEIPT
- Declared conditions:
- Surface or successor produced:
- Delta from expected surface:
- Identity claim: none unless separately established

## Bridge Ledger — when domains are crossed
| Move | Type | Evidence bearing | Promotion limit |
| --- | --- | --- | --- |

Suggested bridge types:
- documented mechanism
- documented association
- scholarly interpretation
- inference
- formal analogy
- metaphor
- theological interpretation
- unresolved bridge

## Pressure
- Quote-to-page check:
- Visual surface / coordinate-space check:
- Edition identity check:
- OCR/HTR disagreement:
- Translation pressure:
- Independence/lineage check:
- Anachronism or memorization check:
- Direct counterexample:
- Nearest boring explanation:
- Independent-domain recurrence:
- Serendipity trap check:
- Replay impersonation check:
- Forced-equilibrium check:
- Ghost-promotion check:

## PRESSURE verdict — when applicable
- Seed:
- Literal verdict: supported | disproved | unresolved | malformed
- What broke:
- What survived:
- Why it survived:
- New predictions:
- Residual weirdness:
- Next discriminator:

## Residual fog
- (record entries here)

## Smallest next discriminators
1.

## Receipt
- Created:
- Researcher or agent:
- Tool and model boundary:
- External byte egress:
- Durable location:
- Promotion: none
```

## Receipt law

- A receipt testifies to what this research run encountered; it does not make
  the claims universally true.
- Generated assertions may depend on generated candidates by lineage, but a
  candidate cannot become its own evidence.
- Claim support preserves the transformation layer actually relied upon. A naked
  page or locus may aid navigation but does not replace an evidence path.
- A spatial citation names the exact visual surface and coordinate space it
  addresses; derived coordinates do not silently migrate to parent images.
- Agreement does not establish independence. Preserve shared ancestry when known
  and `independence: unknown` when it cannot be established.
- Discovery formation and evidence support are different ledgers. A coincidence
  may explain why a search occurred without supporting what the search found.
- Breadcrumbs preserve attributable formation steps; they are not evidence by
  role alone.
- Toast-ghosts preserve inactive formation residue; persistence does not grant
  evidentiary weight or genealogy.
- Replay success does not establish historical identity.
- Add later corrections, refusals, or constitutions as descendants. Do not
  rewrite the original receipt.
- `Promotion: none` means the dossier remains evidence and proposal until a
  named human or owning-world gate admits a consequence.

## PRESSURE law

PRESSURE means “this is insane; prove around it,” not “find support for my
favorite idea.”

- Preserve the original hypothesis verbatim as `H0`.
- Literalization, correction, and cross-domain reformulation create descendants.
- Preserve what each consequential descendant lost as well as what survived.
- Explicitly seek supporting precedent, a direct counterexample, the nearest
  boring explanation, and an independent-domain recurrence.
- Type cross-domain moves in the Bridge Ledger.
- Resemblance does not establish common mechanism, genealogy, or authority.
- A PRESSURE run that cannot return `disproved` is not adversarial research.

> **A crazy hypothesis earns promotion by surviving loss, not by accumulating resemblance.**

## Formation-trace law

When `{ PEEL. SLEEP .LEEP }` is active, read
[formation-trace.md](formation-trace.md).

Keep the governing split visible:

```text
DISCOVERY TRACE — why we looked
EVIDENCE PATH — why we believe, doubt, or refuse
```

`PEEL.` exposes the attributable road and roadside. `SLEEP` suspends one to
three materially live formulations without forcing equality or consensus.
`.LEEP` tests what an attributable receipt can regenerate.

> **PEEL remembers the road. SLEEP lets the roads coexist without lying about
> them. LEEP walks one forward again.**

## Compact answer form

When the full template would bury a simple result, retain:

```text
Finding:
Exact loci:
Source layer inspected:
Reading or translation boundary:
Counterevidence:
Residual fog:
Next discriminator:
```

For a compact PRESSURE run, retain:

```text
SEED:
LITERAL VERDICT:
WHAT BROKE:
WHAT SURVIVED:
WHY IT SURVIVED:
BRIDGE LEDGER:
NEW PREDICTIONS:
RESIDUAL WEIRDNESS:
NEXT DISCRIMINATOR:
```

For a compact formation-trace run, retain:

```text
SURFACE:
BREADCRUMBS:
TOAST-GHOSTS:
SLEEP STATES (1–3):
NEXT DISCRIMINATOR:
.LEEP RESULT:
REPLAY DELTA:
EVIDENCE-PATH BOUNDARY:
```


## Compact DIALOGIC TRACE form

```text
TARGET:
TARGET IDENTITY STATUS:
PUBLIC ARTIFACT:
PRE-QUESTION MODEL:
QUESTION:
LEADING-RISK:
CHANNEL:
PRIVACY / PUBLICATION BOUNDARY:
RESPONSE:
RESPONSE IDENTITY STATUS:
RESPONSE CLASS:
MODEL DELTA:
WHAT CHANGED:
WHAT DID NOT CHANGE:
RESPONSE-DEPENDENT CLAIMS:
INDEPENDENT RETEST:
RESIDUAL DISAGREEMENT:
NEXT DISCRIMINATOR:
PROMOTION: none
```


## Compact ALEX Commons deposit form

```text
DEPOSIT ID:
CONTRIBUTOR MODE:
CARD ID:
TRANSPORT CLASS:
ARTIFACT DIGEST:
RIGHTS TESTIMONY:
HOLD PERMISSION:
PROCESS PERMISSION:
QUOTATION PERMISSION:
PUBLICATION PERMISSION:
IDENTITY DISCLOSURE PERMISSION:
REPLY PERMISSION:
QUARANTINE RESULT:
INGEST RESULT:
INDEPENDENCE STATUS:
AUTHORITY: none
```
